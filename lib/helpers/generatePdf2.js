...
async function generatePdfDocumentation(stack, articleFetcher, firstPages = []) {
  const article = stack[0]
  article.leafArticles = []
  ...
  article.content = article.leafArticles.filter(a => !!a.content).map(a => a.content).join('\n<div class="page-break"></div>\n\n')

  await saveDocumentation(article)
}
...
async function saveDocumentation(article) {
  ...
  const html = await mdToPdf(article, {
    launch_options: { headless: 'new', args: ['--no-sandbox'], executablePath: '/usr/bin/chromium-browser' },
    basedir: './output/',
    as_html: true,
